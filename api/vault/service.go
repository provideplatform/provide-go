...
func EncryptDetached(token, spec, data, publicKey string, opts map[string]interface{}) (*DetachedEncryptDecryptRequestResponse, error) {
	status, resp, err := InitVaultService(common.StringOrNil(token)).Post("encrypt", map[string]interface{}{
		"spec":       spec,
	...
	if status != 200 {
		return nil, fmt.Errorf("failed to encrypt data; status: %v; %s", status, resp)
	}

	r := &DetachedEncryptDecryptRequestResponse{}
	raw, err := json.Marshal(resp)
	if err != nil {
		return nil, fmt.Errorf("failed to encrypt data; status: %v; %s", status, err.Error())
	}
...
func VerifyDetachedSignature(token, spec, msg, sig, publicKey string, opts map[string]interface{}) (*VerifyResponse, error) {
	status, resp, err := InitVaultService(common.StringOrNil(token)).Post("verify", map[string]interface{}{
		"spec":       spec,
		"public_key": publicKey,
		"message":    msg,
		"signature":  sig,
